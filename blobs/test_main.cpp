...
char wBiggestBlob[] = "biggestBlob";
char origShow[] = "origShow";
...
void on_trackbar( int dummy )
{
	...
	IplImage* displayBiggestBlob = 0;
	if(!displayBiggestBlob){
		displayBiggestBlob = cvCreateImage(cvGetSize(original), IPL_DEPTH_8U, 3);
	}

	cvShowImage(origShow, original);
	...
	// ---- Drawing Contour on top of the biggest Blob ----
	cv::Mat mat(displayBiggestBlob);
	
	// My first trial of Drawing Contour on top of the Biggest Blog	
	//cv::Mat* pMat = &mat;
	//mat = cvCreateMat(displayBiggestBlob->height, displayBiggestBlob->width, IPL_DEPTH_8U);
	//cv::Mat& rMat = mat;
	//cvGetMat(displayBiggestBlob, pMat);
	////CvMat *mat = cvCreateMat(displayBiggestBlob->height, displayBiggestBlob->width, IPL_DEPTH_8U);
	////cvConvert(displayBiggestBlob, pMat);
	...
	cv::imshow(wBiggestBlob, mat);


	//cvShowImage(wBiggestBlob, displayBiggestBlob);
...
int main( int argc, char** argv )
{
	...
	//original = cvLoadImage("faceROI.jpeg",0);
	original = cvLoadImage("faceROI.png",0);
